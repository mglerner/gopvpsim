...
- **Bugs #2-#5 + #7: ALL RESOLVED (2026-06-27).** Each was reviewed and
  actioned: #2 float32 damage constants (`f1538ff`), #3 farm-down
  self-debuff stacking (`50e8cd2`), #4 slayer cache focal-level-cap key
  (`4afd397`), #5 bandaid[929] no-bait swap documented as an intentional
  divergence (`cdcef73`), and the `_cm_debuf_delta` dead self-buff branch
  (#7, `30fd819`). Per-bug resolutions below.
...
### 2. [MEDIUM — FIXED] Damage formula uses exact `1.3 / 1.2 / 1.6` constants instead of the game's float32-truncated values
...
- **Resolved (`50e8cd2`):** farm-down now stacks self-debuffs; pinned by `tests/test_bug3_farm_stack.py`.

### 4. [MEDIUM — FIXED] Slayer disk-cache key omits the focal cohort level cap (stale cross-level hits in Master mirror-slayer dives)
...
- **Resolved (`4afd397`):** `focal_max_level` now threaded into `compute_cache_key` (`CACHE_VERSION=4`); pinned by `tests/test_slayer_cache_key.py`.

### 5. [MEDIUM/LOW — RESOLVED] `bandaid[929]` stack-switch is missing the `bait_shields` gate the reference requires (no-bait mode divergence)
...
- **Resolved (`cdcef73`):** kept current behavior, documented as an INTENTIONAL DIVERGENCE (comment at `battle.py:1755-1764`, writeup in `docs/pvpoke_divergences.md` + DEVELOPER_NOTES); pinned by `tests/test_bandaid929_nobait_divergence.py` (the pin fails if the swap is gated, guarding the cold-forcing direction).
...
- `_cm_debuf_delta` self-buff clause dead code (`'1' == 1` string/int mismatch) — real but cosmetic (sweep showed no score impact); FIXED anyway as bug #7 (`30fd819`, pinned by `tests/test_cm_debuf_delta.py`).
