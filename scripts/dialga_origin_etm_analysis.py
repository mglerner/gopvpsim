#!/usr/bin/env python
"""One-off: Dialga (Origin) Master League ETM / IV / best-buddy deep dive.

Background (recommendation conversation 2026-06-19):
  - GoFest lets Michael ETM the signature move (Roar of Time) onto a
    second Dialga (Origin). Two candidate mons in the collection:
      * 15/14/15 - already has Roar of Time
      * 15/15/14 - no special move yet (could ETM it)
  - Decisions to support: build the 15/15/14 here? best-buddy (L51) vs
    regular (L50), on BOTH his side and the opponent's? what does each
    single stat point cost? is the ETM (Roar of Time) worth it vs the
    best no-ETM build?
...
Roar of Time is a strict per-move upgrade over Draco Meteor (both 150/65,
but Draco self-debuffs -2 atk and ROT does not). At the moveset level a
prior screen found ROT/Iron Head and Iron Head/Draco TIE on aggregate
wins, so the ETM's value lives in specific matchups, not the win total --
which is what section 4 surfaces.

Output: a markdown report at userdata/dives/dialga_origin_etm_analysis.md
...
FOCAL_SPECIES = 'Dialga (Origin)'
LEAGUE        = 'master'
POOL_FILE     = 'opponent_pools/master_top60.txt'
OUT_MD        = 'userdata/dives/dialga_origin_etm_analysis.md'

# Curated focal IV set. The two real mons (15/14/15, 15/15/14) ARE the
# -1 def and -1 hp single-point drops from hundo, so the per-stat table
# answers "build the 15/15/14?" directly. 0/15/15 is a deliberately bad
...
FOCAL_IVS = [
    ...
    (14, 15, 15),   # -1 atk
    (13, 15, 15),   # -2 atk
    (15, 14, 15),   # -1 def  == real 15/14/15 (has Roar of Time)
    (15, 13, 15),   # -2 def
    (15, 15, 14),   # -1 hp   == real 15/15/14 (no special move yet)
    (15, 15, 13),   # -2 hp
    (0, 15, 15),    # bad atk, scale anchor
]
# The two real build candidates and the hundo, used in the best-buddy
# matrix (sec 1) and head-to-head (sec 3).
REAL_ROT = (15, 14, 15)   # already has Roar of Time
REAL_ETM = (15, 15, 14)   # ETM candidate
...
# Four movesets, all Dragon Breath. Two ROT builds ("with move") and two
# Iron Head builds (no ETM). Iron Head is the spam/closer in every viable
# build; the second slot is the nuke.
MOVESETS = [
    ('ROT/IH',   'DRAGON_BREATH', ['ROAR_OF_TIME', 'IRON_HEAD']),
    ('ROT/THU',  'DRAGON_BREATH', ['ROAR_OF_TIME', 'THUNDER']),
    ('IH/Draco', 'DRAGON_BREATH', ['IRON_HEAD', 'DRACO_METEOR']),
    ('IH/THU',   'DRAGON_BREATH', ['IRON_HEAD', 'THUNDER']),
]
PRIMARY_MS = 'ROT/IH'   # the build sec 1-3 analyze
...
def section1(results, total):
    ...
    out = ["## 1. Best-buddy 2x2 win matrix\n",
           ...
           f"opponent level) block. Rows = my Dialga-O level (L50 regular, "
           f"L51 best-buddy); columns = opponent level.\n"]
    for iv in [HUNDO, REAL_ROT, REAL_ETM]:
        a, d, s = iv
        tag = {HUNDO: 'hundo', REAL_ROT: 'real, has Roar of Time',
               REAL_ETM: 'real, ETM candidate'}[iv]
        out.append(f"\n**{a}/{d}/{s}** ({tag})\n")
...
def section2(results, total):
    ...
    for label, iv in SENS_ROWS:
        ...
        out.append(f"| {a}/{d}/{s} | {label} | {w50} | "
                   ...
                   f"{w51 - base[diag[1]]:+d} |")
    out.append("\nNote: 15/14/15 (-1 def) and 15/15/14 (-1 hp) are the two "
               "real build candidates, so their rows here equal the totals in "
               "section 1.")
    return "\n".join(out) + "\n"
...
def section3(results):
    ...
    out = ["## 3. 15/14/15 vs 15/15/14 head-to-head\n",
           f"The exact (opponent, shields) matchups where the two real mons "
           f"reach a DIFFERENT outcome under the **{PRIMARY_MS}** build, per "
           f"level block. 15/14/15 has Roar of Time; 15/15/14 is the ETM "
           f"candidate (they differ only by 1 def <-> 1 hp).\n"]
    any_div = False
    for b in LEVEL_BLOCKS:
        ...
        for opp, sh in divs:
            ...
            out.append(f"| {opp} | {fmt_sh(sh)} | {w[m_rot[(opp, sh)]]} | "
                       f"{w[m_etm[(opp, sh)]]} |")
    if not any_div:
        out.append("\n**Conclusion: the two real spreads are functionally "
                    "identical across every block** - the def<->hp swap flips "
                    "no matchups. Build whichever you have; the ETM decision "
                    "(section 4) dominates.")
    return "\n".join(out) + "\n"
...
def section4(results, total):
    """ETM value: with-move (ROT) vs no-ETM (Iron Head) builds, win deltas and
    named flips. Plus the Iron Head vs Thunder second-move coverage contrast."""
    out = ["## 4. ETM value: Roar of Time vs no-ETM builds\n",
           f"All at hundo (15/15/15) vs the {total}-matchup pool. The ETM "
           f"swaps an Iron Head build's nuke for Roar of Time. A prior screen "
           f"found ROT/IH and IH/Draco TIE on aggregate wins, so the value is "
           f"in *which* matchups flip, not the totals.\n"]
    ...
    # Named flips: ROT vs its Iron Head counterpart, two pairings.
    for ms_with, ms_without, desc in [
            ('ROT/IH', 'IH/Draco', 'Roar of Time vs Draco Meteor (Iron Head '
             'as the other charge in both)'),
            ('ROT/THU', 'IH/THU', 'Roar of Time vs Iron Head (Thunder as the '
             'other charge in both)')]:
    ...
    # Second-move coverage: Iron Head vs Thunder, holding ROT fixed.
    out.append("\n### Second charge move: Iron Head vs Thunder (ROT fixed)\n")
    out.append("_Which opponents the Thunder coverage slot turns relative to "
               "Iron Head, with Roar of Time as the primary nuke in both._\n")
    for b in LEVEL_BLOCKS:
        gained, lost = flip_block(results, 'ROT/THU', 'ROT/IH', b)
        if not gained and not lost:
            out.append(f"\n**{blk(b)}**: no matchups flip.")
            continue
        out.append(f"\n**{blk(b)}**: Thunder vs Iron Head: "
                   f"+{len(gained)} / -{len(lost)}:\n")
        if gained:
            out.append("- Thunder GAINS: " +
                       ", ".join(f"{o} {fmt_sh(s)}" for o, s in gained))
        if lost:
            out.append("- Thunder LOSES (Iron Head holds): " +
                       ", ".join(f"{o} {fmt_sh(s)}" for o, s in lost))
    return "\n".join(out) + "\n"
...
def main():
    ...
    header = (
        "# Dialga (Origin) Master League: ETM / IV / best-buddy deep dive\n\n"
        ...
        "Generated by `scripts/dialga_origin_etm_analysis.py`.\n")
