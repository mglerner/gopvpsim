#!/usr/bin/env python
"""Build a standalone focal-vs-opponent IV-robustness page (LOCAL ONLY).

Config-driven generalization of the retired ``build_thievul_licki_page.py`` (S1 of
docs/joint_iv_reuse_plan.md): every pair identity, editorial string and
path comes from a pairs/*.toml config (scripts/joint_iv_config.py). The
Thievul pair configs rebuild the shipped pages byte-identically modulo
the generated timestamp and this script's own name in the footer.

Output: ``userdata/dives/<focal>_vs_<opponent>_iv_robustness.html`` -- a
...
Inputs (all optional under --allow-missing), from the pair's data_dir:
  manifest.json                          grid provenance
  <focal>_<label>__vs__<opponent>.npz    packed win grids
  meta_wins.npz                          per-IV meta wins (A1)
  breakpoints.json                       closed-form damage layer (A2)
  reco.json                              recommendation blob (assembly)
  <denial out_name>.json                 opponent-axis denial (optional)

ALL analysis numbers live in the injected ``TL_DATA`` blob; the page JS
(``scripts/joint_iv_page.js``) only renders them. ``TL_DATA`` and its
side keys (``thievul``/``licki``/``licki_denial``), plus the ``tl-*``
DOM ids, are OPAQUE SLOT NAMES shared with the JS -- they already served
two different opponents and are kept verbatim for every pair; renaming
them would churn a 5k-line contract for zero rendered difference.
"""
import argparse
import base64
...
# ---------------------------------------------------------------------------
# Pair-config-driven module state, set once by _configure(cfg) in main().
# The functions below read these globals (this is a single-run CLI; a
# _configure step keeps the diff against the proven original minimal).
# ---------------------------------------------------------------------------
CFG = None                   # the PairConfig
DATA_DIR = None              # cfg.data_dir (CLI-overridable)
METAWINS_FALLBACK_DIR = None  # [page] metawins_fallback (optional)
FOCAL = None
FOCAL_SHADOW = False
DEFAULT_OPPONENT = None
OPP_SHADOW = False
LEAGUE = None
LEAGUE_LABEL = None
MAX_LEVEL = None
DEFAULT_GRID_LABELS = None   # config grid labels
LABEL_ORDER = None           # [page] label_order, default config grid order
PRIMARY_ARM = None           # first grid label's arm ('iwpr' for thievul)
CLAIM = None                 # [page] claim = {ivs, label}, optional
PRETTY_FALLBACK = None       # derived from config grids
ARCHIVE_NOTE_TEXT = None     # [page] archive_note, optional
INJECTED_MOVES = ()          # cfg.injected_moves
INJECTED_NOTE = None         # [page] injected_note (static, date-anchored)
MAIN_DIVE_URL = None         # [page] main_dive_url, optional
PUBLISH_SLUG_NAME = None     # [page] publish_slug, default derived
CROSSLINK_HTML = ''          # [page] crosslink_html, optional
OCCASION = None              # [page] occasion, optional
MOTIVATION = None            # [page] motivating_question, optional
DESIGN_DOC_FALLBACK = None   # [page] design_doc_fallback, optional
DENIAL_FILENAME = 'denial.json'


def _configure(cfg, data_dir_override=None):
    """Resolve every pair-specific module global from the pair config."""
    global CFG, DATA_DIR, METAWINS_FALLBACK_DIR, FOCAL, FOCAL_SHADOW
    global DEFAULT_OPPONENT, OPP_SHADOW, LEAGUE, LEAGUE_LABEL, MAX_LEVEL
    global DEFAULT_GRID_LABELS, LABEL_ORDER, PRIMARY_ARM, CLAIM
    global PRETTY_FALLBACK, ARCHIVE_NOTE_TEXT, INJECTED_MOVES, INJECTED_NOTE
    global MAIN_DIVE_URL, PUBLISH_SLUG_NAME, CROSSLINK_HTML, OCCASION
    global MOTIVATION, DESIGN_DOC_FALLBACK, DENIAL_FILENAME
    from gopvpsim.pokemon import LEAGUE_MAX_LEVEL
    page = cfg.section('page')
    CFG = cfg
    DATA_DIR = pathlib.Path(data_dir_override) if data_dir_override \
        else cfg.data_dir
    mwf = page.get('metawins_fallback')
    METAWINS_FALLBACK_DIR = (REPO / mwf) if mwf else None
    FOCAL = cfg.focal
    FOCAL_SHADOW = cfg.focal_shadow
    DEFAULT_OPPONENT = cfg.opponent
    OPP_SHADOW = cfg.opp_shadow
    LEAGUE = cfg.league
    LEAGUE_LABEL = cfg.league.capitalize()
    MAX_LEVEL = LEAGUE_MAX_LEVEL[cfg.league]
    DEFAULT_GRID_LABELS = [g.label for g in cfg.grids]
    LABEL_ORDER = list(page.get('label_order') or DEFAULT_GRID_LABELS)
    PRIMARY_ARM = cfg.grids[0].label.rpartition('_')[0] or cfg.grids[0].label
    CLAIM = page.get('claim')
    PRETTY_FALLBACK = {
        g.label: _grid_pretty({'focal_fast': g.focal_fast,
                               'focal_charged': list(g.focal_charged),
                               'bait': g.bait})
        for g in cfg.grids}
    ARCHIVE_NOTE_TEXT = page.get('archive_note')
    INJECTED_MOVES = tuple(cfg.injected_moves)
    INJECTED_NOTE = page.get('injected_note')
    if INJECTED_MOVES and not INJECTED_NOTE:
        raise SystemExit(
            'ABORT: the pair config declares injected_moves but no [page] '
    ...
    DESIGN_DOC_FALLBACK = page.get('design_doc_fallback')
...
def dataset_spec(manifest, data_dir=None):
    ...
    if opp_fast:
        opp_moveset = (
            f"{opp_display} {_pretty_move(opp_fast)} / "
            + ' + '.join(_pretty_move(c) for c in opp_charged)
            + (' (always baits)' if man.get('opp_always_baits') else ''))
    else:
        opp_moveset = f'{opp_display} (moveset not recorded in the manifest)'
    return {
...
def iv_table(species, shadow=False):
...
def won_slice_b64(won, si):
    ...
    mtime=0 pins the gzip header so builds are DETERMINISTIC: the
    original builder used gzip.compress (current-time mtime), which made
    every rebuild differ in 4 bytes per blob for identical data.
    """
...
def load_meta_wins(data_dir, focal_tbl, grid_labels, *, oppiv='pvpoke',
    ...
    if not p.exists():
        # Opponent-independent (the focal vs the dive pool), so a dataset
        # without its own copy borrows a configured sibling's.
        if METAWINS_FALLBACK_DIR is None:
            return None
    ...
    if not np.array_equal(np.asarray(z['iv_rank_ivs'], dtype=int),
        ...
        raise SystemExit('ABORT: meta_wins.npz iv_rank_ivs does not match '
                         f"this build's iv_rank order for "
                         f"{focal_tbl['species']}")
    ...
    for label in grid_labels:
        ...
        if per_scen:
            ...
            found.append(label)
    contract_key = f'wins__{PRIMARY_ARM}__{oppiv}__bait__1-1'
...
def build_collection(spec):
    ...
    rank_lookup = {}
    shadow_of = {spec['focal']: FOCAL_SHADOW,
                 spec['opponent']: OPP_SHADOW}
    for sp in spec['grid_species']:
        shadow = shadow_of.get(sp, False)
        table = compute_rank_lookup(sp, league=LEAGUE, max_level=MAX_LEVEL,
                                    shadow=shadow)
        variant = 'shadow' if shadow else 'normal'
        rank_lookup[sp] = {variant: {f'{a},{d},{s}': r
...
def build_data(data_dir, *, allow_missing, won_labels, won_scenarios,
    ...
    focal_tbl = iv_table(spec['focal'], FOCAL_SHADOW)
    opp_tbl = iv_table(spec['opponent'], OPP_SHADOW)
    ...
    named_builds = []
    if CLAIM:
        named_builds.append({'label': CLAIM['label'],
                             'ivs': list(CLAIM['ivs'])})
    named_builds.append({'label': 'rank 1 (stat product)',
    ...
    uses_injected = any(
        m == g.get('focal_fast') or m in (g.get('focal_charged') or [])
        for m in INJECTED_MOVES
        for g in grids_meta.values())
    if uses_injected:
        notes.append(INJECTED_NOTE)
    ...
    # scripts/joint_iv_page.js. Hand-written parallel prose here is
    ...
    meta = {
        ...
        'provenance': (
            ...
            + (f'Motivating question: {MOTIVATION} ' if MOTIVATION else '')
            + 'Analyzed opponent on this page: '
            + f"{spec['opp_display']}."),
        'notes': notes,
...
def render_page(data, missing, spec):
    ...
    page_js = (REPO / 'scripts' / 'joint_iv_page.js').read_text()
    m = data['meta']
    sims = m['total_sims']
    ...
    occasion_clause = f', for {OCCASION}' if OCCASION else ''
    main_dive_clause = (
    ...
    denial_section = (
        f"<h2>The other side: anti-{spec['focal_display']} "
        f"{spec['opp_display']} "
    ...
    body = BODY_TEMPLATE.format(
        ...
        licki_scope=('heatmap overlay, drill-down and the anti-'
                     + spec['focal_display'] + ' section' if has_denial
...
{theme_head_script()}
<title>{spec['focal_display']} vs {spec['opp_display']} - IV robustness{
    ' (archived)' if archive else ''}</title>
...
def main(argv=None):
    ...
    ap.add_argument('--data-dir', default=None,
                    help='dataset directory override (default: the pair '
                         "config's data_dir)")
    ...
    _configure(cfg, data_dir_override=args.data_dir)
    data_dir = DATA_DIR
    ...
    if args.publish and not pub:
        pub = str(REPO / 'userdata' / 'website' / PUBLISH_SLUG_NAME)
    if pub:
        if missing:
            raise SystemExit(
