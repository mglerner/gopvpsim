...
0. **STANDING PRE-LAUNCH GATE: run `docs/predive_checklist.md`** before every
   ...
   recorded in `docs/TODO_archive.md` (session-3 batch); its findings are folded in.
...
(full batch writeups archived verbatim to `docs/TODO_archive.md`). Each is
non-gating; render/tooling-only ones re-render from replay anytime.

- **[render DRY] score-key `{mi}_{mode}@51` parity (Python<->JS).** Open-coded
  in Python (`deep_dive.py`) and JS (`deep_dive_engine.js`). Consistent today,
...
- **[tooling, silent-incompleteness] verify_overnight UL opponent-count
  ...
  pull would drop from every UL dive, exit 0, and pass the morning gate GREEN.
  Confirmed 0-impact on the current gamemaster (UL 68/68 resolve, 2026-06-27) ->
  LATENT, not a live hole. Needs a marker/expected-count DESIGN decision
  (Michael's call) before asserting, or a false-fail could block the morning gate.
- **[tooling] F3: narrative auto-gen patch is WARN-not-FAIL + unverified**
  (`run_website_dives.py` ~624). Low -- empty narrative blocks are an accepted
  ship state -- but it's the same WARN-not-FAIL shape the ML gate hardened.
  Optional: surface the WARN where `verify_overnight` scans.
- **[tooling] F2: split-moveset page absence invisible to verify_overnight**
  (DOWNGRADED -- the split-emit loop has no try/except, so a *failed* write aborts
  the chain; only a silently *dropped* moveset is uncovered, and a naive count
  guard false-positives on legitimate screening). Leave as-is unless the dive
  emits its actual-surviving count for the verifier to assert against.
- **[render] `#opp-<slug>` canonical landing ("Flavor 2").** A `#opp-` link lands
  on the first-rendered mention (inconsistent target across opponents). Flavor 2 =
  pick one canonical per-opponent target (prefer the `dd-opp-row` detail when
  present, else the breakpoint `<li>`). Links were unused; low priority.
  Render-only, re-render via replay.
- **[engine, low ship-priority] #3-followup: both-self-debuff PvPoke
  divergences.** The bug-#3 verification's 378-cell oracle exposed ~117 PvPoke
  divergences (7 winner-flips) on the broader both-self-debuff population
  (Lurantis, Blaziken, ...) that PRE-DATE #3 (non-default movesets). Likely the
  near-KO-DP / `_optimize_move_timing` self-debuff-timing cluster. Investigate,
  then decide keep-as-divergence vs fix (CLAUDE.md gate). Repro methodology in
  DEVELOPER_NOTES "#2-#6" + `docs/reviews/2026-06-27_engine_bug_hunt.md`.
- **[engine, cosmetic] #6 bandaid[910]** uses defender max-damage move not
  `bestChargedMove` (battle.py); measured cosmetic (16/16 fire-cases match oracle
  on score+winner). Low-priority cleanup, not a shipping bug. (Tracked in
  DEVELOPER_NOTES "#2-#6".)
...
* **Regression tests for the two `c5c515e` port-fidelity fixes (confirmed
  ...
  see DEVELOPER_NOTES / the c5c515e commit). Both are LOW severity (~zero
