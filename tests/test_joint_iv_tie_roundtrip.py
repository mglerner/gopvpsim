"""Round-trip pin for the tie-fact contract between the joint-IV
assembler and the page JS (kit rename of the Thievul/Licki original).

``scripts/joint_iv_assemble.py`` writes a per-card sentence ("2992
spreads tie on the primary metric (1-1 top-512 coverage); tiebreak chain:
...") that ``scripts/joint_iv_page.js`` parses back out in
...
_JS = _SCRIPTS / "joint_iv_page.js"

sys.path.insert(0, str(_SCRIPTS))
sys.path.insert(0, str(_ROOT / "src"))
_spec = importlib.util.spec_from_file_location(
    "joint_iv_assemble", _SCRIPTS / "joint_iv_assemble.py")
...
def test_assembler_builds_every_tie_line_through_the_helper():
    ...
    """
    text = (_SCRIPTS / "joint_iv_assemble.py").read_text()
